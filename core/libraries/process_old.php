...
foreach ($template -> settings -> libraries as $item) {
	...
	if (!empty($settings[1]) && !empty($item[2])) { $path .= $item[2] . DS; }
	...
	if (
		...
		if ($target === 'local' && !empty($settings[1]) && empty($item[2])) {
			init('libraries', 'functions');
			$item[2] = funcLibraries_Version($path);
		}
		...
		foreach ($current[$target] as $k => $i) {
			...
			foreach ($i as $str) {
				...
				if ($target === 'local') {
					$str = $item[1] . '/' . $item[0] . '/' . (!empty($settings[1]) && !empty($item[2]) ? $item[2] . '/' : '') . $str;
				}
