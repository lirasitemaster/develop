...
function sitemap_structureRemap($target, &$array, &$contents) {
	...
	foreach ($target as $key => $item) {
		...
		if (objectIs($item)) {
			sitemap_structureRemap($item, $array, $contents);
		}
...
if (!empty($settings['allow']['inner'])) {
	sitemap_structureRemap($structure, $data['links']['inner'], $contents);
}
