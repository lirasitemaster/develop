...
if (SECURE_RIGHTS) {
	...
	if (
		...
		if (empty($user -> type) || !isALLOW) { $user -> type = 'default'; }
		
		$usersrights = dbUse('rights:' . $user -> type, 'select', ['return' => 'alone:data']);
		...
		if (!empty($usersrights)) {
			...
			logging('rules for \'' . $user -> type . '\' rights group (as type of user) not set in database');
		}
		//} else {
		//	userUnset();
		//	error('403', false, 'rules for \'' . $user -> type . '\' rights group (as type of user) not set in database');
		//}
