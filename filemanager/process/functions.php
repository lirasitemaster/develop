...
function fm_redirect($url = null)
{
    //header('Location: ' . $url, true, $code);
    header('Location: ' . (empty($url) ? FM_SELF_PATH : FM_SELF_URL), true, 302);
    exit;
}
...
function fm_get_translations($tr) {
    try {
        $content = @file_get_contents(DP . 'elements' . DS . 'translation.json');
        if($content !== FALSE) {
...
function fm_convert_win($filename)
{
	...
	if (FM_IS_WIN && mb_detect_encoding($filename) === FM_ICONV_INPUT_ENC) {
        //$filename = iconv(FM_ICONV_INPUT_ENC, 'UTF-8//IGNORE', $filename);
		$filename = mb_convert_encoding($filename, 'UTF-8', FM_ICONV_INPUT_ENC);
    }
    return $filename;
}
